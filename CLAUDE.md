...
- `packages/api` (`@lupira/tasks-api`) — the upstream spec plus the proxy allowlist; see below.
...
`packages/api/exposed.json` names every `VERB /path` the BFF forwards, grouped by the policy it gets
(`operations` → Default, `anonymous` → the account-less share surface). `routes.mjs` writes
`ReverseProxy.Routes` from it — one exact template per path, `Match.Methods` pinned — and
`filter-spec.mjs` filters the spec both clients generate from, so proxy and client cannot disagree.
Both are regenerated by `npm run gen:api`, **never hand-edited**; only `ReverseProxy.Clusters` is
hand-maintained.
