...
import { dueInDays, dueNextWeekend, dueOnDate, formatDue, toDateInputValue } from './dueDate';
...
describe('dueDate', () => {
  ...
  it('uses a short month/day label for further-out dates', () => {
    expect(formatDue(dueInDays(40))?.label).not.toBe('Today');
  });

  it('round-trips a calendar date through end-of-day back to yyyy-mm-dd', () => {
    expect(toDateInputValue(dueOnDate(new Date(2026, 5, 18)))).toBe('2026-06-18');
  });

  it('toDateInputValue is empty for a missing or unparseable date', () => {
